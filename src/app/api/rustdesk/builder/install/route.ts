 ...
 * Tam Yetkili RMM Yükleyici - Sıfır Müdahale, Kesintisiz Bağlantı.
 */
export async function GET(req: Request) {
  try {
    ...
    const psScript = `# --- RUSTDESK TAM YETKILI KURULUM (ZERO-INTERVENTION) ---
...
# 1. Tum RustDesk Sureclerini Temizle
Stop-Process -Name "rustdesk" -Force -ErrorAction SilentlyContinue
Stop-Service -Name "rustdesk" -Force -ErrorAction SilentlyContinue

# 2. TOML Yapılandırması (Unattended Access Ayarları)
$toml = @"
...
Write-Host ">> KURULUM PENCERESI ACILIYOR (Lutfen yuklemeyi tamamlayin)..." -ForegroundColor Yellow
Start-Process $setupPath -Wait

# 4. Sifreyi Enjekte Et ve Servisi Restart Et
$rdExe = "C:\\Program Files\\RustDesk\\rustdesk.exe"
if (Test-Path $rdExe) {
    Write-Host ">> Sifre ve yetki ayarlari yapiliyor..." -ForegroundColor Cyan
    # Servis kapalıyken şifreyi yazdır
    & $rdExe --set-password "$finalPass"
    Start-Service "rustdesk" -ErrorAction SilentlyContinue
    Start-Sleep -Seconds 3
    # Servis açıkken şifreyi teyit et
    & $rdExe --set-password "$finalPass"
    # Ayarların tam oturması için servisi restart et
...
Write-Host ">> KURULUM TAMAMLANDI! Artik sifre sormadan baglanabilirsiniz." -ForegroundColor Green
`;
